  ...
  return (
    <div className={`g3-learning-summary ${levelClass}`}>
      <div className="g3-summary-inner">
        ...
        <button className="g3-secondary-action" type="button" onClick={() => setDetailModalOpen(true)} style={{ margin: '1rem 0', width: '100%', padding: '0.75rem', borderRadius: '0.5rem' }}>
          {language === "th" ? "ดูผลวิเคราะห์ทั้งหมด" : language === "en" ? "View Full Analysis" : "查看完整分析"}
        </button>

        <Group3DetailModal open={detailModalOpen} title={language === "th" ? "ผลวิเคราะห์การเรียน" : language === "en" ? "Learning Analysis" : "学习分析"} onClose={() => setDetailModalOpen(false)}>
