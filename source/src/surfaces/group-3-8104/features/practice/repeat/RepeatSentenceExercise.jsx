      ...
      <article className={`g3-repeat-panel is-${presentation.layout} ${visualSrc ? "has-visual" : "is-text-only"}`} data-phase={liveSession.phase}>
        ...
        <div className="g3-repeat-interaction">
          ...
          {presentation.showFeedback && <div className="g3-repeat-feedback">
              ...
              <div className="g3-repeat-feedback-detail">
                ...
                {pronunciationAnalysis && (
                  <button className="g3-practice-primary is-secondary" type="button" onClick={() => setDetailModalOpen(true)} style={{ marginTop: '1rem', width: '100%' }}>
                    {text.viewDetails || "ดูรายละเอียดการออกเสียง"}
                  </button>
                )}
              </div>
              
              <Group3DetailModal open={detailModalOpen} title={text.viewDetails || "รายละเอียดการฝึก"} onClose={() => setDetailModalOpen(false)}>
                {pronunciationAnalysis && (
            ...
            <div className="g3-practice-actions"><button className="is-secondary" type="button" onClick={() => { setDetailModalOpen(false); retry(); }}>{text.tryAgain}</button><button className="g3-practice-primary" type="button" onClick={() => { setDetailModalOpen(false); send({ type: "NEXT" }); }}>{text.nextExercise} →</button></div>
          </div>}
        </div>
      </article>
    </PracticeExerciseShell>
